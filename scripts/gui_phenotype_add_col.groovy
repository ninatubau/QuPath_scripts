...
startButton.setOnAction {
    ...
    for (def i=0; i< Integer.parseInt(classText.getText()); i++) {
        ...
        criteriaNumber.setOnAction() {
            ...
            //print( criteriaNumber.localToScreen(criteriaNumber.getBoundsInParent()));
           // Float minY = criteriaNumber.localToScreen(criteriaNumber.getBoundsInLocal()).getMinY();
            //Float height = criteriaNumber.localToScreen(criteriaNumber.getBoundsInLocal()).getHeight();
            //val =Math.round(minY/height)-3;
            //print(val);
            ...
            if(cpt>=Integer.parseInt(classText.getText()))
                cpt=0
    ...
    runButton.setOnAction {
        ...
        for (cell in cells){
            ...
            StringBuilder phenotype_total = new StringBuilder();
            //phenotype_total = []
            def ml = cell.getMeasurementList();

            for (phenotype in phenotypes) {
                //print("inside run button 5")
                ...
                ml.putMeasurement(phenotype["name"].toString(), 0)
                if (phenotype_valid) {
                    //No need to check the other phenotypes
                    //cell.setPathClass(phenotype["name"])
                    phenotype_total.append(phenotype["name"].toString()+',');
                    ml.putMeasurement(phenotype["name"].toString(), 1)
            ...
            if (cell_undefined)
                ...
                cell.setPathClass(getPathClass(phenotype_total.toString()));
